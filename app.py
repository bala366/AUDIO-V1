...
CHUNK_TIMEOUT_SECONDS = 240  # 4 minutos por bloco de 30s
CHUNK_MAX_RETRIES = 2
...
def parse_srt_file(path):
    ...
    for block in re.split(r"\\r?\\n\\r?\\n+", text.strip()):
        ...
        try:
            ...
            end = srt_time_to_seconds(right)
        except Exception:
            continue
        spoken = re.sub(r"\\s+", " ", " ".join(lines[time_idx+1:])).strip()
        if spoken:
            records.append({"start":start,"end":end,"text":spoken})
    return records
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.title("Transcritor de Áudio para PDF - V12 ADAPTATIVA")
    ...
    def build_ui(self):
        ...
        ttk.Label(
            top,
            text="TRANSCRITOR DE ÁUDIO PARA PDF - V12 ADAPTATIVA",
        ...
        ttk.Label(
            opts,
            text="V12: blocos de 30s; se travar, divide automaticamente em 15s."
        ...
        ttk.Label(
            self,
            text=(
                "V12 usa SRT, blocos de 30s com fallback de 15s, auditoria e PDF em Downloads. "
    ...
    def audit_path(self):
        if self.current_audio:
            return self.current_audio.with_name(
                self.current_audio.stem + "_AUDITORIA_V12.log"
            )
    ...
    def download_model_resumable(self, model):
        ...
        session.headers.update({"User-Agent":"TranscritorV12/1.0"})
    ...
    def transcribe_piece_to_srt(self, ffmpeg, cli, model, absolute_start, duration, tmpdir, label, timeout_seconds):
        """Converte um trecho do áudio e retorna registros lidos do SRT com tempos absolutos."""
        wav = tmpdir / f"{label}.wav"
        c = subprocess.run(
            [ffmpeg, "-y", "-ss", str(absolute_start), "-t", str(duration),
        ...
        if c.returncode != 0 or not wav.exists():
            raise RuntimeError(f"Falha ao preparar trecho {label}.")

        outbase = tmpdir / f"{label}_resultado"
        cmd = [
            str(cli), "-m", str(model), "-f", str(wav),
            "-l", "pt", "-osrt", "-of", str(outbase)
        ]
        ...
        while True:
            ...
            if elapsed > timeout_seconds:
                ...
                raise TimeoutError(f"Watchdog excedeu {timeout_seconds}s no trecho {label}.")
            try:
                item=lines_queue.get(timeout=1.0)
                if item is None:
                    reader_done=True
                else:
                    raw.append(item.rstrip())
                    self.q.put(("last_event",now()))
            except queue.Empty:
                self.q.put(("status",f"{label}: processo vivo — {int(elapsed)}s — PID {self.proc.pid}"))
        ...
        self.q.put(("pid","-"))
        if code != 0:
            raise RuntimeError(f"whisper.cpp retornou código {code} no trecho {label}.")

        srt_path=Path(str(outbase)+".srt")
        if not srt_path.exists():
            raise RuntimeError(f"Trecho {label} terminou sem gerar SRT.")
        recs=parse_srt_file(srt_path)
        for rec in recs:
            rec["start"] += absolute_start
            rec["end"] += absolute_start
        try:wav.unlink()
        except:pass
        return recs, raw, srt_path

    def preflight_transcription(self, ffmpeg, cli, model, tmpdir):
        """Teste obrigatório de 15 segundos antes de iniciar centenas de blocos."""
        self.q.put(("stage","TESTE INICIAL 15s"))
        self.q.put(("status","Testando 15 segundos antes da transcrição completa..."))
        self.audit_log("Iniciando teste de segurança de 15 segundos")
        recs, raw, srt = self.transcribe_piece_to_srt(
            ffmpeg, cli, model, 0, 15, tmpdir, "teste_inicial", 180
        )
        if not recs:
            raise RuntimeError(
                "TESTE INICIAL FALHOU: o Whisper terminou 15 segundos sem reconhecer fala. "
                "A V12 parou agora e não vai gastar horas processando o áudio inteiro."
            )
        self.audit_log(f"Teste inicial OK: {len(recs)} trecho(s) reconhecido(s)")
        self.q.put(("status",f"TESTE INICIAL OK — {len(recs)} trecho(s). Iniciando blocos..."))

    # ---------------- state ----------------
    def state_path(self):
        return self.current_audio.with_name(
            self.current_audio.stem + "_V12_ESTADO.json"
        )
    ...
    def worker(self):
        ...
        try:
            ...
            tmpdir = Path(tempfile.mkdtemp(prefix="transcritor_v12_"))

            # Teste obrigatório e rápido antes da maratona.
            self.preflight_transcription(ffmpeg, cli, model, tmpdir)

            whisper_log = self.current_audio.with_name(
                self.current_audio.stem + "_WHISPERCPP_V12.log"
            )
            ...
            for idx in range(total_chunks):
                ...
                self.q.put(("stage","WHISPER → SRT 30s"))
                success=False
                last_error=None
                block_records=[]

                # Tenta o bloco principal (até 30 segundos) no máximo 2 vezes.
                for attempt in range(1, CHUNK_MAX_RETRIES+1):
                    ...
                    try:
                        recs, raw, srt_path = self.transcribe_piece_to_srt(
                            ffmpeg, cli, model, start_sec, this_len, tmpdir,
                            f"bloco_{chunk_no:04d}_30s_try_{attempt}",
                            CHUNK_TIMEOUT_SECONDS
                        )
                        block_records=recs
                        success=True
                        with open(whisper_log,"a",encoding="utf-8") as log:
                            log.write(f"\\nBLOCO {chunk_no}/{total_chunks} 30s tentativa {attempt} OK; segmentos={len(recs)}; srt={srt_path}\\n")
                            for ln in raw[-100:]: log.write(ln+"\\n")
                        self.audit_log(f"Bloco {chunk_no}: 30s OK; segmentos={len(recs)}")
                        break
                    except Exception as ex:
                        last_error=str(ex)
                        self.audit_log(f"Bloco {chunk_no}: tentativa 30s {attempt} falhou: {ex}")
                        time.sleep(1)

                # Se 30s não fechar, divide automaticamente em partes de até 15s.
                if not success:
                    self.q.put(("stage","FALLBACK AUTOMÁTICO 15s"))
                    self.q.put(("status",f"Bloco {chunk_no}: 30s falhou. Dividindo automaticamente em 15s."))
                    fallback=[]
                    offset=0.0
                    sub=0
                    fallback_ok=True
                    while offset < this_len - 0.001:
                        sub += 1
                        dur=min(15.0,this_len-offset)
                        sub_success=False
                        sub_error=None
                        for sub_try in range(1,3):
                            self.q.put(("retry",f"15s {sub} tentativa {sub_try}"))
                            try:
                                recs, raw, srt_path = self.transcribe_piece_to_srt(
                                    ffmpeg, cli, model, start_sec+offset, dur, tmpdir,
                                    f"bloco_{chunk_no:04d}_15s_{sub}_try_{sub_try}",
                                    150
                                )
                                fallback.extend(recs)
                                sub_success=True
                                self.audit_log(f"Bloco {chunk_no} subparte {sub}: OK; segmentos={len(recs)}")
                                break
                            except Exception as ex:
                                sub_error=str(ex)
                                self.audit_log(f"Bloco {chunk_no} subparte {sub} tentativa {sub_try} falhou: {ex}")
                        if not sub_success:
                            fallback_ok=False
                            last_error=f"Subparte {sub} falhou: {sub_error}"
                            break
                        offset += dur
                    if fallback_ok:
                        block_records=fallback
                        success=True

                if not success:
                    raise RuntimeError(
                        f"Bloco {chunk_no} falhou até no modo adaptativo de 15 segundos.\\nÚltimo erro: {last_error}"
                    )
    ...
    def write_pdf(self):
        ...
        story = [
            ...
            Paragraph("<b>Motor:</b> whisper.cpp — V12 Adaptativa",body),
            Paragraph(
